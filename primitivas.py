...
def comparaPolares(ancora, ponto):
	"""Retorna o ângulo polar entre o ponto âncora e o ponto dado"""
	dx = ponto[0] - ancora[0]
	dy = ponto[1] - ancora[1]
	angulo = math.atan2(dy, dx)
	return angulo

def comparaDistancias(ancora, ponto):
	"""Retorna a distância entre o ponto âncora e o ponto dado"""
...
def segmentosInterceptam(seg1, seg2):
	...
	if (d1==1 and d2==2) or (d1==2 and d2==1) and \
	   (d3==1 and d4==2) or (d3==2 and d4==1):
		return True
	elif (d1==0 and onSegment(seg2, seg1[0])) or \
		 (d2==0 and onSegment(seg2, seg1[1])) or \
		 (d3==0 and onSegment(seg1, seg2[0])) or \
		 (d4==0 and onSegment(seg1, seg2[1])):
		return True
	return False